...
def build_mlp(input_size, output_size, n_layers, size):
    ...
    class Model(nn.model):
        def __init__(self):
            ...
            self.layers.append(nn.Sequential(nn.Linear(input_size, size[0]), nn.ReLU()))
            for i in range(n_layers - 1): self.layers.append(nn.Sequential(nn.Linear(size, size), nn.ReLU))
            self.layers.append(nn.Sequential(nn.Linear(size, output_size)))
    ...
    return Model(input_size, output_size, n_layers, size)
